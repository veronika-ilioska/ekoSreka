        ...
        <div class="row g-4 align-items-stretch">
          <div v-for="q in beginner" :key="q.id" class="col-12 col-sm-6 col-lg-4">
            <RouterLink :to="`/quizzes/${q.id}`" class="poster-link">
              <article class="quiz-poster" :style="{ '--bg': `url(${poster(q)})` }" role="button" :aria-label="q.title">
                <div class="overlay"></div>
                <div class="content">
                  <h5 class="title mb-2">{{ q.title }}</h5>
                  <p class="desc text-white-50 mb-3">{{ q.description }}</p>
                  <span class="chip">Започни <img class="icon" src="../img/guide.svg" alt="" /></span>
                </div>
              </article>
            </RouterLink>
          </div>
...
.poster-link { text-decoration: none; display: block; }
...
.overlay{
  ...
  background: linear-gradient(180deg, rgba(0,0,0,.12) 0%, rgba(0,0,0,.35) 45%, rgba(0,0,0,.6) 100%);
  z-index: 0;
}
...
.content{
  ...
  z-index: 1; text-shadow: 0 1px 2px rgba(0,0,0,.45);
}
.title{ 
  font-weight: 800;
   letter-spacing: .2px;
...
@media (min-width: 992px){
  .quiz-poster{ min-height: 300px; }
  .title{ font-size: 1.6rem; }
}
.icon { width: 18px; height: 18px; vertical-align: -2px; margin-right: 6px; }
</style>
