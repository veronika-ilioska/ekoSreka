...
import com.zinemasterapp.ekosrekja.quiz.repo.QuizOptionRepository;
import com.zinemasterapp.ekosrekja.quiz.repo.QuizQuestionRepository;
import com.zinemasterapp.ekosrekja.quiz.repo.QuizRepository;
import com.zinemasterapp.ekosrekja.quiz.repo.QuizSubmissionRepository;
...
public class QuizController {
    ...
    @GetMapping
    public Page<dto.QuizListItem> list(@RequestParam(required = false) String level,//ako nema level site kvizovi gi davame
                                       @PageableDefault(size=12) Pageable p){
        Page<Quiz> page = (level == null || level.isBlank())
                ? quizRepo.findByActiveTrue(p)
                : quizRepo.findByActiveTrueAndLevelIgnoreCase(level, p);

        return page.map(q -> new dto.QuizListItem(q.getId(), q.getTitle(), q.getLevel(), q.getDescription(), q.getTimeMinutes()));
    }
    ...
    public Map<String,Object> getQuiz(@PathVariable Long id){
        ...
        List<dto.QuizQuestionDto> qdto = questions.stream().map(q -> {
            List<QuizOption> opts = qoRepo.findByQuestionIdOrderByOrdAsc(q.getId());//gi barame options na prashanjata
            var optDtos = opts.stream()
                    .map(o -> new dto.QuizQuestionDto.OptionDto(o.getId(), o.getText(), o.getOrd()))
                    .toList();
    ...
    public dto.QuizResultDto submit(@PathVariable Long id, @RequestBody List<AnswerIn> answers){
        ...
        List<QuizQuestion> questions = qqRepo.findByQuizIdOrderByIdAsc(id);//koj kviz(gi zemame negovite prashanja)
        Map<Long, List<QuizOption>> optionsByQ = questions.stream()//za site prashanja gi zemame options
                .collect(Collectors.toMap(QuizQuestion::getId, q -> qoRepo.findByQuestionIdOrderByOrdAsc(q.getId())));//{id:List<Options>}
        ...
        for (QuizQuestion q : questions) {
            Long qid = q.getId();
            Long chosen = answers.stream()
                    .filter(a -> a.questionId.equals(qid))
                    .map(a -> a.optionId)
                    .findFirst()
                    .orElse(null);

            Long correct = optionsByQ.get(qid).stream()
